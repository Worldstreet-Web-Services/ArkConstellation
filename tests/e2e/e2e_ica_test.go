...
func (s *IntegrationTestSuite) testICARegisterAccountAndSendTx() {
	s.Run("register_ICA_account_and_send_tx_to_chainB", func() {
		...
		s.Require().Eventually(
			func() bool {
				icaAccountBalances, err = queryAllBalances(chainBAPIEndpoint, icaAccount)
		...
		s.Require().Eventually(
			func() bool {
				recipientBalances, err = queryAllBalances(chainBAPIEndpoint, recipientB)
				s.Require().NoError(err)
...
func (s *IntegrationTestSuite) registerICAAccount(c *chain, valIdx int, sender, connectionID, fees string) {
	...
	s.executeTxCommand(ctx, c, icaCmd, valIdx, s.defaultExecValidation(c, valIdx))
