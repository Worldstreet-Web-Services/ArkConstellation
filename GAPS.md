...
- **Cosmos SDK fork's non-mechanical behavior changes have never received an independent semantic review — unowned, no deadline.** PR #25 (the v0.53.6→v0.53.8 re-pin) mechanically re-confirmed these are unchanged from the original fork baseline but explicitly left its own "Independent state-machine/security review" checkbox unchecked, since the mechanical advisory closure it exists to deliver doesn't depend on it. Nothing in this repo currently owns or schedules this review — `track/3-security-chaos`'s completed sign-off (`scripts/chaos/reports/day3-final-signoff.md`) covered mempool/consensus/circuit-breaker/contract testing, not these three items, and `docs/decisions/module-and-config-decisions.md` doesn't mention any of them. The three items needing an explicit "yes, this is right for this chain" decision from whoever owns `x/bank`/`x/mint`/`x/auth` (Eng 1 / this track's lead, unless reassigned):
  1. **`x/bank` BeforeSend hooks** (`TrackBeforeSend`/`BlockBeforeSend`) — inert no-op today, but live attack-surface/extension-point plumbing in the money-movement hot path the moment anything calls `SetHooks`.
...
- **Cosmos SDK fork version drift — RESOLVED as of this pin.** The fork was measurably behind upstream on two real fixes (secp256k1 pubkey-tag validation, compact-bitarray bounds check — both landed in v0.53.7/v0.53.8, past the fork's then-current v0.53.6 pin). Remediated by advisory `GHSA-93g8-x7xx-r6w2`: the pin moved to `v0.53.8-v8-mantra-1` (commit `58cc2eb4a66fcab2359a69d262b984958c8d9273`), which carries the full v0.53.7/v0.53.8 backport set. `cosmossdk.io/api` was re-pinned in lockstep to the same commit (now checked automatically by `scripts/check-sdk-api-lockstep.sh`). Upstream `release/v0.53.x` continues to accrue unreleased commits past v0.53.8 — worth re-checking for drift at the next SDK re-pin, not assumed closed forever. Full detail: `docs/proof/fork-audit-cosmos-sdk.md`.
