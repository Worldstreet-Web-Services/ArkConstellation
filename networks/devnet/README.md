...
| `govtimelock.execution_delay` | `30000000000` (30s) | **Devnet-fast** (production leaves this `0`, meaning the module default of 48h, decision #14) | The `x/govtimelock` module (see `GAPS.md`'s resolved-gaps section and `x/govtimelock/README.md`) enforces the locked ≥48h execution delay; this genesis activates it (`activation_height: 1`) with a short delay so a submit→vote→pass→execute cycle is observable inside a working session, same reasoning as `gov.voting_period` above. `GenesisState.Validate()` would reject a nonzero delay this low on a production genesis, but `InitGenesis` intentionally does not enforce that minimum so this override works — see the `ExecutionDelay` field comment in `x/govtimelock/types/genesis.go`. |
...
| `evm.params.active_static_precompiles` | 9 addresses (`app.StaticPrecompileAddresses`) | **Required, not optional — same on both networks** | `arkd init`'s raw default is an empty list, because this app never sets `evmtypes.DefaultStaticPrecompiles`. An empty list disables every static precompile (staking, distribution, ICS20, bank, gov, slashing, bech32, p256, distrclaim) and calls to them *succeed while doing nothing* — no error, real gas burned. That was live on `arkdevnet_9000-1` from genesis until governance proposal #8 (see `proposals/activate-static-precompiles.json` for the incident). The set must equal exactly what `app/app.go` registers with the EVM keeper: `app/precompiles_test.go` checks every genesis fixture in the repo against the live keeper. |
