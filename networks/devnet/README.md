...
| `evm.params.active_static_precompiles` | 9 addresses (`0x…0100`, `0400`, `0800`, `0801`, `0802`, `0804`, `0805`, `0806`, `0a01`) | **Same on both networks** | Issue #37. `arkd init`'s raw default is an empty list - `App.DefaultGenesis()` populates it but `genutilcli.InitCmd` never calls that, so without this override no Cosmos module is reachable from Solidity. Kept identical to the mainnet set so contract-level testing here is representative. **`0x…0803` (Vesting) is deliberately absent**: the fork advertises the address but ships no implementation, and activating it makes `GetStaticPrecompileInstance` panic on first call - `ValidatePrecompiles` and `validate-genesis` both accept it, so nothing else will stop you. See `docs/decisions/proposals/precompile-enablement-proposal.md`. |
