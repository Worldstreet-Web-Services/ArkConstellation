...
|---|---|---|---|
| `staking.unbonding_time` | `1814400s` (21 days) | **Mainnet-shaped** (matches locked decision #10) | Changed from `300s` on 2026-09-03. The fast value made IBC unusable: a counterparty's light-client trusting period is bounded by our unbonding period, so `300s` capped it at ~200s and any relayer interruption past that would expire the client permanently — recoverable only by governance *on the counterparty chain*. Interop now outranks in-session unbonding tests on this network; use a throwaway chain for those. See `docs/decisions/proposals/ibc-interop-unbonding-proposal.md`. |
